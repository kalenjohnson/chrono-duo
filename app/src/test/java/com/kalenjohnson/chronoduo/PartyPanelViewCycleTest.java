...
/**
 * JVM coverage for {@link PartyPanelView#cycleStepForward}/{@link
 ...
 * again -- an apparent no-op. {@link PartyPanelView#cycleStepBack} already
 * used real modulo and was never affected; it's covered here for symmetry.
 */
public class PartyPanelViewCycleTest {
    ...
    public void forwardStepsThroughEveryChoiceThenWrapsToNone() {
        ...
        int total = 4;
        assertEquals(1, PartyPanelView.cycleStepForward(0, total));
        assertEquals(2, PartyPanelView.cycleStepForward(1, total));
        assertEquals(3, PartyPanelView.cycleStepForward(2, total)); // last choice -> None
        // The bug: stepping forward from None (curIdx == total - 1) must
        // wrap to the first choice (0), not silently produce None again.
        assertEquals(0, PartyPanelView.cycleStepForward(3, total));
    }
    ...
    public void backStepsThroughEveryChoiceThenWrapsFromNone() {
        ...
        assertEquals(2, PartyPanelView.cycleStepBack(3, total));
        assertEquals(1, PartyPanelView.cycleStepBack(2, total));
        assertEquals(0, PartyPanelView.cycleStepBack(1, total));
        assertEquals(3, PartyPanelView.cycleStepBack(0, total)); // wraps to None
    }
    ...
    public void forwardAndBackAreInverses() {
        ...
        for (int i = 0; i < total; i++) {
            assertEquals(i, PartyPanelView.cycleStepBack(PartyPanelView.cycleStepForward(i, total), total));
            assertEquals(i, PartyPanelView.cycleStepForward(PartyPanelView.cycleStepBack(i, total), total));
        }
    }
}
